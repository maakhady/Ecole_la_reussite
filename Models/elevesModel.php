...
class EleveModel {
    ...
    public function getAllEleves($page = 1, $eleveParPage = 10) {
        $offset = ($page - 1) * $eleveParPage;
        $sql = "SELECT id, matricule, nom, prenom, date_naissance, nom_tuteur, tel_tuteur, email_tuteur, departement, classe, date_inscription, archivage
            ...
            $stmt->bindValue(':limit', $eleveParPage, PDO::PARAM_INT);
    ...
    public function compterTousLesEleves() {
        $sql = "SELECT COUNT(*) as total FROM eleves WHERE archivage = 0";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute();
