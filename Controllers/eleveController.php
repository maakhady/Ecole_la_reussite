...
class EleveController {
    ...
    public function listEleves() {
        try {
            ...
            $eleveParPage = 10;   // Nombre d'élèves par page

            $eleves = $this->eleveModel->getAllEleves($page, $eleveParPage);
            $totalEleves = $this->eleveModel->compterTousLesEleves();
            $elevesprimaires = $this->eleveModel->compterElevesPrimaire();
            $elevessecondaires = $this->eleveModel->compterElevesSecondaire();
            $totalPages = ceil($totalEleves / $eleveParPage);
